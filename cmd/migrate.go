...
import (
	...
	"github.com/oatsaysai/simple-go-http-server/db/migrations"

	log "github.com/oatsaysai/simple-go-http-server/log"
)

var migrateDirectoryDbCmd = &cobra.Command{
	Use: "migrate-db",
...
func init() {
	rootCmd.AddCommand(migrateDirectoryDbCmd)

	migrateDirectoryDbCmd.Flags().Int("number", -1, "the migration to run forwards until; if not set, will run all migrations")
	migrateDirectoryDbCmd.Flags().Bool("dry-run", false, "print out migrations to be applied without running them")
}
